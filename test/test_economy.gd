...
func test_first_tower_costs_base_price() -> bool:
	assert_eq(EconomySim.tower_price(&"basic", 0), 35, "first basic")
	assert_eq(EconomySim.tower_price(&"long", 0), 165, "first long range")
	return true

func test_price_escalates_per_tower_already_owned() -> bool:
	assert_eq(EconomySim.tower_price(&"basic", 1), 135, "35 + 100")
	assert_eq(EconomySim.tower_price(&"basic", 3), 335, "35 + 3*100")
	assert_eq(EconomySim.tower_price(&"mortar", 2), 655, "115 + 2*270")
	return true
...
func test_tower_price_looks_up_the_given_kind_not_a_fixed_one() -> bool:
	assert_eq(EconomySim.tower_price(&"fast", 0), 80,
...
func test_first_tower_of_every_kind_costs_exactly_its_base_cost() -> bool:
	assert_eq(EconomySim.tower_price(&"basic", 0), 35, "basic base cost")
	assert_eq(EconomySim.tower_price(&"fast", 0), 80, "fast base cost")
	assert_eq(EconomySim.tower_price(&"mortar", 0), 115, "mortar base cost")
	assert_eq(EconomySim.tower_price(&"long", 0), 165, "long base cost")
	return true

# Escalation step size differs per kind (100/150/270/400) — pin each kind's own
...
func test_escalation_step_matches_each_kinds_own_cost_escalation() -> bool:
	assert_eq(EconomySim.tower_price(&"fast", 1), 230, "80 + 150")
	assert_eq(EconomySim.tower_price(&"long", 1), 565, "165 + 400")
	return true
...
func test_escalation_is_linear_and_isolated_per_kind_at_higher_counts() -> bool:
	assert_eq(EconomySim.tower_price(&"basic", 7), 735, "35 + 7*100")
	assert_eq(EconomySim.tower_price(&"fast", 7), 1130, "80 + 7*150")
	assert_eq(EconomySim.tower_price(&"long", 4), 1765, "165 + 4*400")
	assert_eq(EconomySim.tower_price(&"mortar", 4), 1195, "115 + 4*270")
...
func test_negative_owned_count_clamps_to_base_price() -> bool:
	assert_eq(EconomySim.tower_price(&"basic", -1), 35,
		"a negative count must not push the price below base")
	assert_eq(EconomySim.tower_price(&"long", -5), 165,
...
func test_sell_refund_on_an_escalated_price_exceeds_base_price_refund() -> bool:
	...
	assert_eq(base_price, 35, "sanity: base price")
	assert_eq(escalated_price, 335, "sanity: escalated price")
	...
	assert_eq(EconomySim.sell_refund(escalated_price), 167, "half of 335, rounded down")
	return true
