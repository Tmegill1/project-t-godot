...
# technique test_map_renderer.gd uses for _GRID_LINE_COLOR and test_tower.gd/
# test_projectile.gd use for their own constants: the alpha values _draw()
# would pass to draw_circle/draw_arc are plain script-level consts on a
# class_name script, directly readable with no tree, node, or draw pass.
